...
const Carousel: FC<Props> = ({ products }) => {
  ...
  return (
    ...
    >
      {products.length > 1 ? (
        <Button onClick={handleLeftOnClick}>
          <IconLeft />
        </Button>
      ) : null}
      <CarouselContainer to={`/game/${products[index].id_product}`}>
        <Image
          src={products[index].thumbnail_product}
          alt={products[index].name_product}
        />
        <CarouselSubContainer>
          <div
            style={{
          ...
          >
            <h1>
              {products[index].name_product.length < 10
                ? products[index].name_product
                : products[index].name_product.slice(0, 10) + "..."}
            </h1>
      ...
      {products.length > 1 ? (
        <Button onClick={handleRightOnClick}>
          <IconRight />
        </Button>
      ) : null}
    </div>
